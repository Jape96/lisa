...
class NetworkPerformance(TestSuite):
    ...
    def _notify_passthrough_line_rate_summary(
        ...
        result_summary = (
            f"Passthrough line-rate: peak {best_throughput_gbps:.2f} Gbps "
            f"({line_rate_percent:.2f}% of {line_rate_gbps:.2f} Gbps), "
            f"threshold {threshold_gbps:.2f} Gbps, sample [{best_sample}], "
            f"client_nic={client.name}:{client_nic_name}, "
            f"server_nic={server.name}:{server_nic_name}"
        )
        test_result.set_status(test_result.status, result_summary)
        client.log.info(
